...
describe("User visits the deletes item confirmation page", () => {
    ...
    it("renders a confirmation paragraph, a 'yes' button and a 'no' link", () => {
        assert.equal(browser.getText(".delete-item-confirmation p"), "Você está certo que deseja deletar este item?");
        assert.equal(browser.getText(".delete-item-confirmation button"), "SIM");
        assert.equal(browser.getText(".delete-item-confirmation a", "src"), "Não");
    });
